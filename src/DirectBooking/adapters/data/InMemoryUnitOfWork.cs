...
namespace DirectBooking.adapters.data
{
    ...
    {
        private Dictionary<Guid, RoomBooking> _bookings = new Dictionary<Guid, RoomBooking>();
       ...
       /// <param name="ct">Token to allow cancelling the ongoing operation</param>
        public Task<RoomBooking> GetAsync(Guid bookingId, CancellationToken ct = default(CancellationToken))
        {
            var tcs = new TaskCompletionSource<RoomBooking>();
            _bookings.TryGetValue(bookingId, out RoomBooking value);
            tcs.SetResult(value);
            return tcs.Task;
         }
